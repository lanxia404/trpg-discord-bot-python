#!/usr/bin/env python3
"""
測試清理和重新組織後的 TRPG Discord Bot 架構
"""
...
def test_new_structure():
    ...
    try:
        ...
        print("✓ cogs.dice_commands 導入成功")
    except ImportError as e:
        print(f"✗ cogs.dice_commands 導入失敗: {e}")
        return False
    
    try:
        from cogs.skill_commands import SkillCommands
        print("✓ cogs.skill_commands 導入成功")
    except ImportError as e:
        print(f"✗ cogs.skill_commands 導入失敗: {e}")
        return False
    
    try:
        from cogs.log_commands import LogCommands
        print("✓ cogs.log_commands 導入成功")
    except ImportError as e:
        print(f"✗ cogs.log_commands 導入失敗: {e}")
        return False
    
    try:
        from cogs.admin_commands import AdminCommands
        print("✓ cogs.admin_commands 導入成功")
    except ImportError as e:
        print(f"✗ cogs.admin_commands 導入失敗: {e}")
        return False
    
    try:
        from cogs.help_commands import HelpCommands
        print("✓ cogs.help_commands 導入成功")
    except ImportError as e:
        print(f"✗ cogs.help_commands 導入失敗: {e}")
        return False
